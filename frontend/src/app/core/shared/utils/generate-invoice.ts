...
const generatePDF = async (
  ...
  const tableBody = [
    ...
    ...orderDetails.map((item) => [
      ...
      { text: `${currency}${(item.subtotal || 0).toFixed(2)}`, alignment: "right" },
    ]),
  ];
  ...
  content.push({
    columns: [
      { text: "", width: "*" },
      {
        width: "auto",
        stack: [
          { text: "Shipping", alignment: "left" },
          { text: `${currency}${(order.shippingCost || 0).toFixed(2)}`, alignment: "right" },

          { text: "Subtotal", alignment: "left" },
          { text: `${currency}${(order.subtotal || 0).toFixed(2)}`, alignment: "right" },

          { text: "Discount", color: "#10b981" },
          { text: `- ${currency}${(order.totalDiscount || 0).toFixed(2)}`, alignment: "right", color: "#10b981" },

          { text: "Tax (IVA)" },
          { text: `${currency}${(order.totalTax || 0).toFixed(2)}`, alignment: "right" },
