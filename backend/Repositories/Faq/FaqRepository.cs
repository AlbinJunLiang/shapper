...
namespace Shapper.Repositories.Faqs
{
    public class FaqRepository : IFaqRepository
    {
        ...
        public async Task<List<Faq>> GetByStoreCodeAsync(string storeCode)
        {
            return await _context.Faqs
                .Include(f => f.Store)
                .Where(f => f.Store != null && f.Store.StoreCode == storeCode)
                .OrderByDescending(f => f.CreatedAt)
                .ToListAsync();
        }
        ...
        public async Task<(List<Faq> Faqs, int TotalCount)> GetPaginatedByStoreCodeAsync(
            int page,
            int pageSize,
            string? storeCode = null)
        {
            ...
            {
                query = query.Where(f => f.Store != null && f.Store.StoreCode == storeCode);
            }
        ...
        {
            return await _context.Faqs.CountAsync(f => f.StoreId == storeId);
        }

        public async Task<int> CountByStoreCodeAsync(string storeCode)
        {
            return await _context.Faqs
                .CountAsync(f => f.Store != null && f.Store.StoreCode == storeCode);
        }
