...
<div class='header'>
    ...
    <p><strong>REFERENCIA:</strong> {WebUtility.HtmlEncode(order.OrderReference ?? "-")}</p>
        ...
        private static string BuildTotals(OrderResponseDto order, ExtraDataDto? extra)
        {
            ...
            return $@"
<div class='totals'>
    <p><strong>Costo de Envío:</strong> {CURRENCY}{order.ShippingCost:F2}</p>
    <p><strong>Subtotal:</strong>  {CURRENCY}{order.Subtotal:F2}</p>
    <p><strong>Impuestos:</strong>  {CURRENCY}{order.TotalTax:F2}</p>
    <p><strong>Descuento:</strong>  {CURRENCY}{order.TotalDiscount:F2}</p>
    <p><strong>Total Final:</strong>  {CURRENCY}{order.Total:F2}</p>
</div>";
        }
